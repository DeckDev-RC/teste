...
class AntiCacheHelper {
   ...
   * @param {string} provider - Provedor de IA ('gemini' ou 'openai')
   * @returns {Object} Configuração de geração única
   */
  static generateAntiCacheConfig(provider = 'gemini') {
    const baseConfig = {
  ...
  /**
   * Gera configurações específicas para retry com variações progressivas
   ...
   */
  static getRetryConfig(attempt = 0, provider = 'gemini') {
    ...
    if (provider === 'gemini') {
      ...
      return {
        ...baseConfig,
        max_tokens: 4096,
        top_p: 0.95 + baseVariation + (Math.random() * 0.02),
      };
    }
   ...
   */
  static applyFullAntiCache(originalPrompt, fileName = '', fileIndex = null, attempt = 0, provider = 'gemini') {
    ...
    const generationConfig = this.getRetryConfig(attempt, provider);
