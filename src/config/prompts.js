...
**REGRAS GERAIS:**
Formato da data:
- Sempre use o padrão DD-MM (ex: 22-06 para 22 de junho).
- Utilize a data informada no comprovante, como data da compra, transferência, pagamento ou Pix realizado.

Separador decimal:
- O valor deve ser formatado com ponto como separador decimal (ex: 200.00, 1600.00), com duas casas decimais obrigatórias.

Campos obrigatórios:
- DATA → conforme tipo do documento (data do Pix, pagamento, transferência, etc.)
- NOME → fornecedor, recebedor ou favorecido (conforme o caso)
- DESCRIÇÃO → virá sempre do nome do arquivo (entre parênteses)
- VALOR → valor total pago ou transferido

Nomes não identificados:
- Se o nome do fornecedor ou recebedor não estiver visível, retornar “ND”.
- Exemplo: 28-06 ND (MANUTENÇÃO) 95.00

Descrição:
- Sempre usar a descrição do nome do arquivo, entre parênteses.
- Deve ser breve e clara: ex: (COMBUSTÍVEL), (SALÁRIO), (GRÁFICA)

Formato final obrigatório:
DD-MM NOME (DESCRIÇÃO) XXX.XX

Retorno único e limpo:
- Não inclua explicações, quebras de linha ou títulos.
- Retorne apenas a string no formato especificado.

Casos especiais:
- Mesmo que um comprovante tenha estrutura incomum (ex: maquininha diferente), siga o padrão acima.
...
Prompt:
Se não houver nome visível de cliente ou conta de origem, assuma:

DATA
DESCRIÇÃO: VENDA PIX
VALOR
FORMATO DE RETORNO:
DD-MM VENDA PIX VALOR

EXEMPLO:
02-07 VENDA PIX 100,00
...
-Todos os documentos em “Contas a Receber” representam receitas efetivamente recebidas
        ...
        PAYMENT: `Identifique os tipos de documentos financeiros e extraia os dados conforme especificado:
